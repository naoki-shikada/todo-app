...
- ゴール：優先度がつけられて絞り込みができるようになる。それを curl / ブラウザで確認できる。（※ curlで確認する部分とブラウザで確認する部分を分けて一文にまとめるのは検討中）
- 分解：STEP1（DB：priority列を追加） → STEP2（API実装：POST/GET/PATCHの基本形） → STEP3（絞り込み機能：?priority=1） → STEP4（フロントエンド：表示・設定UI） → STEP5（テスト・ドキュメント整備：まとめて実施）
- 検証：
  - STEP1：`\d todos` で priority 列が表示される
  - STEP2：curl で POST/GET/PATCH を実行し、レスポンスに priority が正しく含まれる
  - STEP3：`curl .../todos?priority=1`（および2, 3も）で該当するpriorityのtodoだけ返る
  - STEP4：ブラウザで新規作成・一覧表示・編集の3操作を行い、優先度が正しく反映される
  - STEP5：
    - カバレッジ（8番）：`npm test -- --coverage` を実行し、「All files」の合計値が70%以上であればOK
    - architecture.md（10番）：以下3点が実際のコードと一致していることを確認する
      1. テーブル構造の表に priority 列が追加されている（型・デフォルト値の説明含む）
      2. APIの説明に絞り込み仕様（?priority=）が追加されている
      3. フロントエンドの説明に優先度機能が追加されている
- 失敗予測：
  - priorityに1,2,3以外の値（0や"高"などの文字列）が来たときの挙動
  - 既存のtodoデータ（priority列を追加する前のレコード）の扱い
  - STEP5を最後にまとめる方式のため、STEP1〜4のどこにバグがあったか原因特定がしにくくなるリスク（実装時にメモを残しておくと楽になる）
- 失敗予測との比較（実施結果）：
  - priorityに1,2,3以外の値 → `validatePriority()`で整数かつ1・2・3のいずれかのみ許可する実装にし、`0`・`4`・`"高"`・`1.5`（小数）を400で弾くテストを追加して確認済み
  - 既存レコードの扱い → マイグレーションで`DEFAULT 1`を指定し、priority列追加前の既存行にも自動で`1`（低）が入るようにした
  - STEP5を最後にまとめるリスク → 実際にはSTEP2実装直後に`npm test`を実行し、既存テスト1件（POSTの新規作成テスト）がその場で壊れているのを検知・即修正できた。まとめて実施ではなく、各STEP後すぐにテストを回す方が原因特定しやすいと分かった
